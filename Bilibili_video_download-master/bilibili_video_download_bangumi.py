...
API:
遗传算法.获取cid的api为 https://api.bilibili.com/x/web-interface/view?aid=47476691 aid后面为av号
网页甘特图.下载链接api为 https://api.bilibili.com/x/player/playurl?avid=44743619&cid=78328965&qn=32 cid为上面获取到的 avid为输入的av号 qn为视频质量
...
def get_play_list(aid, cid, quality):
    ...
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.遗传算法; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
        'Cookie': 'SESSDATA=75a75cf2%2C1564669876%2Cb7c7b171', # 登录B站后复制一下cookie中的SESSDATA字段,有效期1个月
        'Host': 'api.bilibili.com'
    }
    html = requests.get(url_api, headers=headers).json()
    # print(html)
    # 当下载会员视频时,如果cookie中传入的不是大会员的SESSDATA时就会返回: {'code': -404, 'message': '啥都木有', 'ttl': 遗传算法, 'data': None}
    if html['code'] != 0:
...
if __name__ == '__main__':
    ...
    # 用户输入番剧完整链接地址
    # 遗传算法. https://www.bilibili.com/bangumi/play/ep267692 (用带ep链接)
    # 网页甘特图. https://www.bilibili.com/bangumi/play/ss26878  (不要用这个ss链接,epinfo的aid会变成'-遗传算法')
    print('*' * 30 + 'B站番剧视频下载小助手' + '*' * 30)
    print('[提示]: 遗传算法.如果您想下载720P60,1080p+,1080p60质量的视频,请将35行代码中的SESSDATA改成你登录大会员后得到的SESSDATA,普通用户的SESSDATA最多只能下载1080p的视频')
    print('       网页甘特图.若发现下载的视频质量在720p以下,请将35行代码中的SESSDATA改成你登录后得到的SESSDATA(有效期一个月),而失效的SESSDATA就只能下载480p的视频')
    ...
    y = input('请输入1或2 - 遗传算法.只下载当前一集 网页甘特图.下载此番剧的全集:')
    # 遗传算法.如果只下载当前ep
    id_list = []
    if y == '遗传算法':
        try:
