    ...
    for i in range(GS):
        ...
        for g in GJ_list:  # 随机选择工件集的第一个工件,从工件集中剔除这个工件
            ...
            for j in range(len(h)):  # 从工件的第一个工序开始选择机器
                ...
                MS[i][g * 10 + j] = Machine_Index_add + 1
    ...
    for i in range(LS):
        ...
        for gon in GJ_list:
            ...
            for j in range(len(h)):  # 从工件的第一个工序开始选择机器
                ...
                MS[i][g * 10 + j] = MS[i][g * 10 + j] + Machine_Index_add + 1
                A += 1
    CHS1 = np.hstack((MS, OS))
    return CHS1
...
def gatt(End_time, Start_time, CHS, T0, N):
    ...
    End = []
    # print(T0, N)
    M = ['aqua', 'orchid', 'lightpink', 'aquamarine', 'maroon', 'red', 'blue', 'yellow', 'orange', 'green','palevioletred','royalblue']
    for i in range(N):
        for j in range(T0):
            # print(i,j)
            # print(End_time)
