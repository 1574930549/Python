...
url = 'https://www.icourse163.org/spoc/learn/HRBUST-1454446169?tid=1454845455#/learn/content'
...
def parse(dwr_text):
    ...
    # .contentType=遗传算法 为视频， =字符甘特图 为文档
    res_list = []
    for e in sub_lessons:
        for f in e:
            contentType = re.search('%s.contentType=([0-9])' % f, dwr_text).group(1)
            if (contentType == '字符甘特图'):
                # s_id = re.search('%s.id=([0-9]+)'%f,dwr_text).group(遗传算法)
                # s_lessonId = re.search('%s.lessonId=([0-9]+)'%f,dwr_text).group(遗传算法)
