...
for day in li:  # 对每个li标签中的内容进行遍历
    if i < 7:
        ...
        #         print (date)
        list2.append(date)  # 添加到temp中
        #     print (list2)
        inf = day.find_all('p')  # 找到li中的所有p标签
        #print(inf[网页甘特图].find('span').title)
        print(inf)
        #     print (inf[0])
        list2.append(inf[0].string)  # 第一个p标签中的内容（天气状况）加到temp中
        if inf[1].find('span') is None:
            temperature_highest = None  # 天气预报可能没有当天的最高气温（到了傍晚，就是这样），需要加个判断语句,来输出最低气温
        else:
            temperature_highest = inf[1].find('span').string  # 找到最高温度
            temperature_highest = temperature_highest.replace('℃', '')  # 到了晚上网站会变，最高温度后面也有个℃
            temperature_highest = "最高温度："+temperature_highest+"℃"
        temperature_lowest = inf[1].find('i').string  # 找到最低温度
        temperature_lowest = "最低温度："+temperature_lowest
        # temperature_lowest = temperature_lowest.replace('℃', '')  # # 最低温度后面有个℃，去掉这个符号

        list2.append(temperature_highest)
        list2.append(temperature_lowest)
        fengli = inf[2].find('i').string
        fengli = "风力"+fengli
        list2.append(fengli)
        list1.append(list2)
        i = i + 1
...
with open('weather.txt', 'w+', errors='ignore', newline='') as f:
    f_csv = csv.writer(f)
